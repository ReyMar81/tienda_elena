...
const form = useForm({
    ...
    valor_descuento_decimal: props.promocion.valor_descuento_decimal,
    fecha_inicio: props.promocion.fecha_inicio?.split("T")[0],
    fecha_fin: props.promocion.fecha_fin?.split("T")[0],
...
onMounted(() => {
    // Cargar productos con pivot data
    if (props.promocion.productos) {
        productosSeleccionados.value = props.promocion.productos.map((p) => ({
            id: p.id,
            nombre: p.nombre,
            codigo: p.codigo,
            aplica_mayorista: p.pivot?.aplica_mayorista || false,
            aplica_minorista: p.pivot?.aplica_minorista || false,
        }));
    }

    // Cargar categorías con pivot data
    if (props.promocion.categorias) {
        categoriasSeleccionadas.value = props.promocion.categorias.map((c) => ({
            id: c.id,
            nombre: c.nombre,
            aplica_mayorista: c.pivot?.aplica_mayorista || false,
            aplica_minorista: c.pivot?.aplica_minorista || false,
        }));
    }
});
...
<template>
    ...
    <AppLayout :title="`Editar: ${promocion.nombre}`">
        <FlashNotification />
                ...
                <div class="col-lg-12">
                    ...
                    >
                        <div>
                            ...
                            <p class="text-muted">
                                Actualiza la información de la oferta o
                                descuento
                            </p>
                        </div>
                        <Link
